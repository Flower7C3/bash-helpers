...
function symfony_database_migrate() {
    ...
    has_migrations=$(${symfony_console} doc:mig:sta --no-ansi | grep "New Migrations" | sed 's/[^0-9]//g')
    ...
    if [[ -n "$symfony_console" ]]; then
        if [[ "$has_migrations" != "0" ]]; then
            ...
            printf "${COLOR_CONSOLE}%S doctrine:migrations:migrate${COLOR_OFF}\n\n" "$symfony_console"
...
function mysql_remote_truncate_via_symfony() {
    ...
    if [[ -n "$host_name" ]]; then
        if [[ -n "$symfony_console" ]]; then
            printf "${COLOR_INFO_B}Truncate database at ${COLOR_INFO_H}%s${COLOR_INFO_B} host with ${COLOR_INFO_H}%s${COLOR_INFO_B}${COLOR_INFO}\n" "$host_name" "$symfony_console"
...
function mysql_remote_query_via_symfony() {
    ...
    printf "${COLOR_INFO_B}Run query at ${COLOR_INFO_H}%s${COLOR_INFO_B} in ${COLOR_INFO_H}%s${COLOR_INFO_B} directory${COLOR_INFO}\n" "$host_name" "$symfony_root_dir"
...
function mysql_remote_export_via_symfony() {
    ...
    printf "${COLOR_INFO_B}Backup MySQL database at ${COLOR_INFO_H}%s${COLOR_INFO_B} host in ${COLOR_INFO_H}%s${COLOR_INFO_B} director (via Symfony config)${COLOR_INFO}\n" "$host_name" "$symfony_root_dir"
...
function mysql_remote_import_via_symfony() {
    ...
    printf "${COLOR_INFO_B}Import MySQL database at ${COLOR_INFO_H}%s${COLOR_INFO_B} host in ${COLOR_INFO_H}%s${COLOR_INFO_B} directory (via Symfony config)${COLOR_INFO}\n" "$host_name" "$symfony_root_dir"
...
function mysql_backup_via_symfony() {
    ...
    backup_file_name="${sql_host}.${sql_base}.$(date "+%Y%m%d.%H%M%S").sql"
