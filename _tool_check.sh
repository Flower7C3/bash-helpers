...
function file_name_check() {
    ...
    file_name_test_1=$(echo -n "$file_name" | grep -E "/")
...
function dir_name_check() {
    ...
    output_dir_test_1=$(echo -n "$dir_name" | grep -E "/$")
...
function domain_status_code_check() {
    ...
    local userpass=$2
    local statusCode
    statusCode=$(curl -H "Cache-Control: no-cache" -L -u "$userpass" -s -o /dev/null -I -w "%{http_code}" "$domain")
...
function domain_status_code_check_or_rollback() {
    ...
    local exists
    exists=$(curl -k -s --head "$url" | head -n 1 | grep "HTTP/1.[01] [23]..")
    ...
    if [ "$exists" == "" ]; then
        ...
        printf "${COLOR_SUCCESS_B}Site url ${COLOR_SUCCESS_H}%s${COLOR_SUCCESS_B} is working ${COLOR_SUCCESS}\n" "$url"
    fi
}
